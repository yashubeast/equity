...
class BankDB:
    ...
    def get_last_message(self, discord_id):
        self.cursor.execute(
            """
            SELECT Timestamp FROM LastMessage WHERE DiscordID = ?""",
            (discord_id,),
        )
        result = self.cursor.fetchone()
        if result is None:
    ...
    def delete_message(self, message_id):
        self.cursor.execute(
            "SELECT DiscordID, Value FROM Messages WHERE MessageID = ?", (message_id,)
        )
        result = self.cursor.fetchone()
        if result:
    ...
    def contract_add(self, contractor, contractee, amount, days):
        now = datetime.utcnow()
        self.cursor.execute(
            """INSERT INTO Contracts (Contractor, Contractee, Amount, Days) VALUES (?,?,?,?)""",
            (contractor, contractee, amount, days),
        )
        self.connection.commit()
    ...
    def add_message(self, discord_id, message_id, coins):
        self.cursor.execute(
            """
            INSERT INTO Messages (MessageID, DiscordID, Value) VALUES (?,?,?)""",
            (message_id, discord_id, coins),
        )
        self.connection.commit()
